...
def get_criterion(p):
    if p['criterion'] == 'simclr':
        ...
        entropy_weight = p['criterion_kwargs'].get('entropy_weight', 0.0)
...
def get_model(p, pretrain_path=None):
    # Get backbone
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] == 'partnet':
            from models.resnet_partnet import resnet18
            backbone = resnet18()
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    if p['train_db_name'] == 'partnet':
        ...
        if p['train_type_name'] == 'chair':
            dataset = PARTNET(split='train', type='chair', transform=transform)
        elif p['train_type_name'] == 'table':
            dataset = PARTNET(split='train', type='table', transform=transform)
        elif p['train_type_name'] == 'bed':
            dataset = PARTNET(split='train', type='bed', transform=transform)
        elif p['train_type_name'] == 'bag':
            dataset = PARTNET(split='train', type='bag', transform=transform)
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    if p['train_db_name'] == 'partnet':
        from data.partnet import PARTNET
        if p['train_type_name'] == 'chair':
            dataset = PARTNET(split='test', type='chair', transform=transform)
        elif p['train_type_name'] == 'table':
            dataset = PARTNET(split='test', type='table', transform=transform)
        elif p['train_type_name'] == 'bed':
            dataset = PARTNET(split='test', type='bed', transform=transform)
        elif p['train_type_name'] == 'bag':
            dataset = PARTNET(split='test', type='bag', transform=transform)
