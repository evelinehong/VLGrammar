...
def main():
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        clustering_stats = hungarian_evaluate(lowest_loss_head, predictions, val_dataset.dataset.classes, compute_confusion_matrix=True)
        ...
        torch.save({'optimizer': optimizer.state_dict(), 'model': model.state_dict(), 
                    'epoch': epoch + 1, 'best_loss': best_loss, 'best_loss_head': best_loss_head},
                     p['scan_checkpoint'])
        torch.save({'model': model.module.state_dict(), 'head': best_loss_head}, "%s_%d"%(p['scan_model'], clustering_stats['ACC'] * 100))
